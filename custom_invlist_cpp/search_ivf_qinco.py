...
import Qinco.datasets as datasets 
from Qinco.codec_qinco import encode
from Qinco.utils import (
    add_to_ivfaq_index,
...
# clone Qinco as a subdirectory (or link)
sys.path.insert(0, "Qinco")
...
def run_search(args, ds, index, res):
    ...
    if args.redo_search > 1:
        print(f"redoing {args.redo_search} times")
        experiments = experiments * args.redo_search
    ...
    for cno in experiments:
        ...
        t0 = time.time()
        if args.defer_id_decoding: 
            decode_1by1 = args.id_decoding_1by1
            D, I, codes = index.search_defer_id_decoding(
                xq, nshort, decode_1by1=decode_1by1, return_codes=2)
        else: 
...
def main():
    ...
    aa(
        "--id_compression",
        default="none",
        choices="none packed-bits elias-fano roc wavelet-tree wavelet-tree-1".split(),
        help="How to compress the ids",
    )  
    aa("--defer_id_decoding", default=False, action="store_true")
    # aa("--id_decoding_1by1", default=False, action="store_true")
    aa("--redo_search", default=1, type=int, help="numer of times to redo the search (to stabilize timings)")
    ...
    # set automatically 
    args.id_decoding_1by1 = args.id_compression != "roc"
    ...
    if "search" in args.todo:
        ...
        if args.id_compression != "none": 
            print("compressing ids with", args.id_compression)
            t0 = time.time()
            if args.id_compression == "packed-bits": 
                il = custom_invlists.CompressedIDInvertedListsPackedBits(index.invlists)
            elif args.id_compression == "elias-fano": 
                il = custom_invlists.CompressedIDInvertedListsEliasFano(index.invlists)
            elif args.id_compression == "roc": 
                il = custom_invlists.CompressedIDInvertedListsFenwickTree(index.invlists) 
            elif args.id_compression == "wavelet-tree":  
                il = custom_invlists.CompressedIDInvertedListsWaveletTree(index.invlists) 
            elif args.id_compression == "wavelet-tree-1": 
                il = custom_invlists.CompressedIDInvertedListsWaveletTree(index.invlists, 1)
            else: 
                assert False
            t1 = time.time()
            print(f"compressed ids size: {il.compressed_ids_size_in_bytes} bytes, compressed in {t1-t0:.3f} s")
            res.compressed_ids_size_in_bytes = il.compressed_ids_size_in_bytes
            res.id_compression_time = t1 - t0
            index.own_invlists 
